...
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'SiljeTerje_Hilsener')
CREATE TABLE SiljeTerje_Hilsener (id INT IDENTITY(1,1) PRIMARY KEY, navn NVARCHAR(100) NOT NULL, tekst NVARCHAR(MAX) NOT NULL, opprettet DATETIME2 DEFAULT GETDATE());
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'SiljeTerje_Toaster')
CREATE TABLE SiljeTerje_Toaster (id INT IDENTITY(1,1) PRIMARY KEY, navn NVARCHAR(100) NOT NULL, epost NVARCHAR(200) NULL, tema NVARCHAR(200) NULL, melding NVARCHAR(MAX) NOT NULL, opprettet DATETIME2 DEFAULT GETDATE());
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'SiljeTerje_Besok')
CREATE TABLE SiljeTerje_Besok (id INT IDENTITY(1,1) PRIMARY KEY, navn NVARCHAR(100) NULL, side NVARCHAR(50) NULL, ua NVARCHAR(500) NULL, ip NVARCHAR(64) NULL, opprettet DATETIME2 DEFAULT GETDATE());
    ...
    try {
        ...
        const perNavn = await executeQuery(connection, `
            SELECT b.navn,
                   ...
                   MIN(b.opprettet) AS forste,
                   (SELECT TOP 1 ip FROM SiljeTerje_Besok WHERE navn = b.navn AND ip IS NOT NULL ORDER BY opprettet DESC) AS ip
        ...
        try {
            highscore = await executeQuery(connection, `
                ...
                FROM SiljeTerje_Highscore GROUP BY navn ORDER BY score DESC, sist ASC`);
        } catch {}
        let spillTopp = [];
        try {
        ...
        context.log.error('siljeterje-stats error:', err.message);
