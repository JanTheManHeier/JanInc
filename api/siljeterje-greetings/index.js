...
CREATE TABLE SiljeTerje_Hilsener (
    ...
    try {
        ...
        if (req.method === 'GET') {
            const rows = await executeQuery(connection,
                'SELECT TOP 200 id, navn, tekst, opprettet FROM SiljeTerje_Hilsener ORDER BY opprettet DESC');
        ...
        if (req.method === 'POST') {
            ...
            await executeQuery(connection,
                'INSERT INTO SiljeTerje_Hilsener (navn, tekst) VALUES (@navn, @tekst)',
                [
        ...
        context.log.error('siljeterje-greetings error:', err.message);
