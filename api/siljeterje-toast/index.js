...
CREATE TABLE SiljeTerje_Toaster (
...
module.exports = async function (context, req) {
    ...
    try {
        ...
        await executeQuery(connection,
            'INSERT INTO SiljeTerje_Toaster (navn, epost, tema, melding) VALUES (@navn, @epost, @tema, @melding)',
            [
        ...
        if (!mailRes.ok) {
            context.log.warn('siljeterje-toast: e-postvarsel feilet:', mailRes.reason);
        }

        context.res = { status: 200, headers, body: { success: true, mailSent: mailRes.ok } };
    } catch (err) {
        context.log.error('siljeterje-toast error:', err.message);
