...
(function () {
  ...
  async function vekkDb() {
    try {
      await fetchMedTimeout(`${API_BASE}/siljeterje-greetings`, {}, 30000);
    } catch {}
  }
  ...
  function deviceId() {
    let id = localStorage.getItem('siljeterje-device');
    if (id) return id;
    ...
    localStorage.setItem('siljeterje-device', id);
    return id;
  }
  ...
  function initHjem() {
    ...
    // QR-kode til appen (easyqrcodejs)
    const qrEl = document.getElementById('qr-kode');
    if (qrEl && window.QRCode) {
      try {
        qrEl.innerHTML = '';
        new QRCode(qrEl, {
          text: 'https://janinc.no/SiljeOgTerje/',
          width: 220,
          height: 220,
          colorDark: '#1A2C3F',
          colorLight: '#FAF6EE',
  ...
  function renderHero() {
    _set('hero-navn', `${HERO.brud} & ${HERO.brudgom}`);
    _set('hero-tagline', HERO.tagline);
    _set('hero-dato', HERO.datoTekst);
    _set('hero-sted', HERO.stedTekst);
    _set('hjem-om-tittel', HERO.omTittel);
    _set('hjem-om-tekst', HERO.omTekst);
    _set('hjem-kleskode', HERO.kleskode);
  }

  function renderOmoss() {
    if (typeof OMOSS === 'undefined') return;
    _set('omoss-tittel', OMOSS.tittel);
    _set('omoss-tekst', OMOSS.tekst);
    const f = document.getElementById('omoss-forlovelse');
    if (f) f.textContent = OMOSS.forlovelse || '';
  }

  function renderPraktisk() {
    const el = document.getElementById('praktisk-info');
    if (!el) return;
    el.innerHTML = (PRAKTISK || []).map(p => `<div>${esc(typeof p === 'string' ? p : (p.tittel ? p.tittel + ': ' : '') + (p.tekst || ''))}</div>`).join('');
  }

  function renderMeny() {
    const el = document.getElementById('meny-list');
    if (!el) return;
    el.innerHTML = (MENY || []).map(m => `
      <div class="meny-rett">
        ${m.tag ? `<div class="meny-tag">${esc(m.tag)}</div>` : ''}
        <div class="meny-ikon">${esc(m.ikon || '🍽️')}</div>
        <h3 class="meny-tittel">${esc(m.tittel || '')}</h3>
        ${(m.beskrivelse || m.tekst) ? `<p class="meny-beskrivelse">${esc(m.beskrivelse || m.tekst)}</p>` : ''}
        ${(m.drikke || []).map(d => `<div class="meny-drikke"><span></span><div>${esc(d)}</div></div>`).join('')}
      </div>`).join('');
    const info = document.getElementById('meny-info');
    if (info && typeof MENYINFO !== 'undefined') {
      info.innerHTML = (MENYINFO || []).map(l => `<div>${esc(l)}</div>`).join('');
    }
  }

  function renderGave() {
    _set('gave-tittel', GAVE.tittel);
    _set('gave-intro', GAVE.intro);
    const onsk = document.getElementById('gave-onsker');
  ...
  // Henter redigerbart innhold som Silje & Terje har lagret i admin
  async function lastContent() {
    try {
      const r = await fetchMedTimeout(`${API_BASE}/siljeterje-content`, {}, 60000);
      if (r.ok) {
        const data = await r.json();
        const c = data && data.content;
        if (c && typeof c === 'object') {
          if (c.hero) Object.assign(HERO, c.hero);
          if (c.omoss && typeof OMOSS !== 'undefined') Object.assign(OMOSS, c.omoss);
          if (Array.isArray(c.menyinfo) && typeof MENYINFO !== 'undefined') { MENYINFO.length = 0; c.menyinfo.forEach(l => MENYINFO.push(l)); }
          if (Array.isArray(c.program) && c.program.length) { PROGRAM.length = 0; c.program.forEach(p => PROGRAM.push(p)); }
  ...
  function oppdaterGjesterStats() {
    ...
    const toast = GJESTER.filter(g => (g.rolle || '').toLowerCase() === 'toastmaster').length;
    stats.innerHTML = `
      <div><strong>${totalt}</strong>Gjester</div>
      <div><strong>${toast}</strong>Toastmastere</div>`;
  }
  ...
  function renderGjester() {
    ...
    if (gjesterFilter === 'brudepar') liste = liste.filter(g => ['brud', 'brudgom'].includes((g.rolle || '').toLowerCase()));
    else if (gjesterFilter === 'toast') liste = liste.filter(g => (g.rolle || '').toLowerCase() === 'toastmaster');
    else if (gjesterFilter === 'forlover') liste = liste.filter(g => (g.rolle || '').toLowerCase() === 'forlover');
    ...
    const _rolleRank = g => {
      const r = (g.rolle || '').toLowerCase();
      return r === 'brud' ? 0 : r === 'brudgom' ? 1 : r === 'toastmaster' ? 2 : r === 'forlover' ? 3 : 9;
    };
    liste.sort((a, b) => {
      const ra = _rolleRank(a), rb = _rolleRank(b);
    ...
    grid.innerHTML = liste.map((g, idx) => {
      ...
      const rolle = (g.rolle || '').toLowerCase();
      const erToast = rolle === 'toastmaster';
      if (erToast) klasser.push('toast');
      if (g.avbud) klasser.push('avbud');
      if (rolle === 'brud' || rolle === 'brudgom') klasser.push('jubilant');
      let tag = '';
      if (rolle === 'brud') tag = `<span class="gjest-tag jubilant">👰 Brud</span>`;
      else if (rolle === 'brudgom') tag = `<span class="gjest-tag jubilant">🤵 Brudgom</span>`;
      else if (erToast) tag = `<span class="gjest-tag toast">🎤 Toastmaster</span>`;
      else if (rolle === 'forlover') tag = `<span class="gjest-tag">🌸 Forlover</span>`;
  ...
  function visGjestModal(g) {
    ...
    else if (rolle === 'toastmaster') tag = `<span class="gjest-tag toast">🎤 Toastmaster</span>`;
    else if (rolle === 'forlover') tag = `<span class="gjest-tag">🌸 Forlover</span>`;
    else if (g.avbud) tag = `<span class="gjest-tag avbud">Avbud</span>`;
    else if (g.plus) tag = `<span class="gjest-tag">+${g.plus} følge</span>`;
    innhold.innerHTML = `
  ...
  async function lastHilsener() {
    ...
    try {
      const res = await fetchMedTimeout(`${API_BASE}/siljeterje-greetings`, {}, 60000);
  ...
  async function sendHilsen() {
    ...
    try {
      const res = await fetch(`${API_BASE}/siljeterje-greetings`, {
        method: 'POST',
  ...
  async function sendToast() {
    ...
    try {
      const res = await fetch(`${API_BASE}/siljeterje-toast`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  // ============ Sang ============
  function initSang() {
    const l1 = document.getElementById('lyrics-text-1'); if (l1) l1.textContent = SANG1_LYRICS;
    const l2 = document.getElementById('lyrics-text-2'); if (l2) l2.textContent = SANG2_LYRICS;
    document.querySelectorAll('[data-lyrics-toggle]').forEach(b => {
      b.onclick = () => {
        const box = document.getElementById(b.dataset.lyricsToggle);
        if (!box) return;
        box.hidden = !box.hidden;
        b.textContent = box.hidden ? '📜 Vis sangtekst' : '🙈 Skjul tekst';
      };
    });
  }
  ...
  function visResultat() {
    ...
    let tekst = '';
    if (prosent === 100) tekst = 'Perfekt! Du kjenner Silje & Terje usedvanlig godt 🥇';
    else if (prosent >= 80) tekst = 'Imponerende! Du er definitivt en ekte venn 🥈';
    else if (prosent >= 60) tekst = 'Bra! Du kjenner brudeparet godt 🥉';
  ...
  async function sendHighscore(navn, score, antall) {
    try {
      await fetch(`${API_BASE}/siljeterje-highscore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function lastTopp() {
    ...
    try {
      const r = await fetchMedTimeout(`${API_BASE}/siljeterje-highscore`, {}, 60000);
      const data = await r.json();
      if (!data.length) {
  ...
  function renderBord() {
    ...
    for (const nr of bordNumre) {
      ...
      for (const g of b.gjester) {
        ...
        const erPar = ['brud','brudgom'].includes((g.rolle||'').toLowerCase());
        const navnTag = erPar ? `<span class="jubilant-tag">💍</span>` : '';
        html += `<div class="bord-gjest${treff ? ' bord-gjest-treff' : ''}${erPar ? ' bord-gjest-jubilant' : ''}" data-bord-idx="${idx}">
  ...
  function initMario() {
    ...
    BobleSpill.init(canvas, async (score) => {
      ...
      let tekst = '';
      if (score >= 1000) tekst = '🥇 Helt rått! Du er en ekte boble-mester.';
      else if (score >= 500) tekst = '🥈 Imponerende! Kjærligheten seiret.';
      else if (score >= 200) tekst = '🥉 Bra jobbet! Mange bobler poppet.';
      else if (score > 0) tekst = '💕 Du fanget noen — prøv igjen for full pott!';
  ...
  async function lastMarioTopp() {
    ...
    try {
      const r = await fetchMedTimeout(`${API_BASE}/siljeterje-spillscore`, {}, 60000);
      const data = await r.json();
      if (!data.length) {
        liste.innerHTML = '<div class="empty-state"><div class="empty-icon">🎮</div><div>Ingen scores ennå — bli den første!</div></div>';
        return;
  ...
  async function sporBesok(side) {
    try {
      ...
      await fetchMedTimeout(`${API_BASE}/siljeterje-track`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          navn: navnTilSporing,
  ...
  async function lastMatches() {
    ...
    try {
      const r = await fetch('matches.json?v=20260821-1', { cache: 'no-store' });
